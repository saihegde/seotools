...
public class ListDemo {
	...
	private static final String[] WELCOME_MESSAGE = { "Using this demo, you can see how rows can be conveniently inserted,",
			...
			"top row, put headers, such as 'Name', 'Day', 'Address', or any other", "important piece of information.  A good example might be:", "",
			"    Name      Day      Phone", "    Rosa      Tue      555-1212", "    Vladimir  Wed      555-3137", "    Sanjay    Thu      555-2127",
			"    Ejike     Fri      555-4444", "", "It is best practices to freeze the top row, in the 'Sorting' tab.",
			"This way, sorting will make sure the top row stays intact.  We", "suggest you leave your browser open while trying this out.", "",
			"It is important to know each row is identified by GData by a", "random-looking string like 'czhdp', which stays the same forever,",
			"even if the row is moved or changed.  This type of identifier", "is used throughout this demo. ", "" };

	/** Help on all available commands. */
	private static final String[] COMMAND_HELP_MESSAGE = { "Commands:", " load                   [[select a spreadsheet and worksheet]]",
			" list                   [[shows all entries]]", " reverse                [[shows all entries in reverse order]]",
			" add name=Rosie,day=Tue [[adds an entry]]", " delete <id>            [[deletes; use 'list' first]]",
			" update <id> day=Wed    [[modifies]", " search Rosie Tue       [[full-text search]]", " query name = 'Rosie'   [[structured query]]",
			" exit" };
	 ...
	 * because Google Data has a feature where if an entry is updated between
	 * the time you download it and update or delete it, you will be alerted of
	 * the edit. For example:
	 * 
	 * 1. In this sample app, I look at the row for Rosie, with age 26. 2.
	 * Someone editing the spreadsheet changes Rosie's age to 29 3. In this
	 * sample app, I try to change Rosie's age to 24
	 * 
	 ...
	 * proper entry ID from Google Data, update that fresh entry, and post. In
	 * that case, no caching is necessary, and a version conflict is less
	 * likely. On the other hand, you won't get alerted if someone else has
	 * changed the entry in the meantime.
	 * 
	 ...
	 * 
	 * @param entryToChange
	 *            the entry to change based on the parameters
	 * @param nameValuePairs
	 *            the list of name/value pairs, containing the name of the title
	 *            (in lowercase with all invalid characters removed), .
	 */
	 ...
	 * 
	 * @param nameValuePairs
	 *            pairs such as "name=Rosa,phone=555-1212"
	...
	public void printAndCacheEntry(ListEntry entry) {
		...
		// I.E., this turns http://spreadsheets.google.com/..../cpzh6 into
		// cpzh6.
	...
	/**
	 * Displays the given list of entries and prompts the user to select the
	 * index of one of the entries. NOTE: The displayed index is 1-based and is
	 ...
	 * @param type
	 *            describes the tyoe of things the list contains
	 * @return the 0-based index of the user's selection
	 * @throws IOException
	 *             if an I/O error occurs while getting input from user
	 */
	 ...
	 * all the given words.
	 * 
	 * @param fullTextSearchString
	 *            a string like "Rosa 555" will look for the substrings Rosa and
	 *            555 to appear anywhere in the row
	 * @throws ServiceException
	 *             when the request causes an error in the Google Spreadsheets
	 *             service.
	 * @throws IOException
	 ...
	 * 
	 * This looks up the old cached row so that the version ID is known. A
	 * version ID is used by GData to avoid edit collisions, so that you know if
	 * someone has changed the row before you delete it.
	 * 
	 * For this reason, the cached version of the row, as you last saw it, is
	 * kept, instead of querying the entry anew.
	 * 
	 * @param idToDelete
	 *            the ID of the row to delete such as "cph6n"
	 * @throws ServiceException
	 *             when the request causes an error in the Google Spreadsheets
	 *             service.
	 * @throws IOException
	 ...
	 * @param nameValuePairs
	 *            the name value pairs, such as "name=Rosa" to change the row's
	 *            name field to Rosa
	 * @throws ServiceException
	 *             when the request causes an error in the Google Spreadsheets
	 *             service.
	...
	public static void main(String[] args) throws AuthenticationException {
		String username = "adkasad";
		String password = "asdnakldsa";
		boolean help = false;
